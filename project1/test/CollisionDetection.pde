...
void ballBall(Ball ball1, Ball ball2){
  if (ball1.center.distanceTo(ball2.center) <= (ball1.radius + ball2.radius)){
    Vec2 norm = (new Vec2(ball1.center.x- ball2.center.x,ball1.center.y - ball2.center.y)).normalized();
    float v_b1 = dot(ball1.v, norm);
    float v_b2 = dot(ball2.v, norm);
    float norm_v_b1 = (ball1.mass * v_b1 + ball2.mass * v_b2 - ball2.mass * (v_b1 - v_b2) * cor);
    float norm_v_b2 = (ball1.mass * v_b1 + ball2.mass * v_b2 - ball1.mass * (v_b2 - v_b1) * cor);
    updateCenter(ball1,ball2,norm);
    ball1.v.add(norm.times(norm_v_b1/(ball1.mass + ball2.mass) - v_b1));
    ball2.v.add(norm.times(norm_v_b2/(ball1.mass + ball2.mass) - v_b2));
  }
}
